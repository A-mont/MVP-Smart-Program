...
fn main() {
    ...
    let idl_path = cargo_toml_path.clone().join("mvp_contract.idl");
    let client_path = outdir_path.clone().join("mvp_client.rs");
    ...
    fs::copy(client_path, cargo_toml_path.join("mvp_client.rs"))
        .unwrap();
}
